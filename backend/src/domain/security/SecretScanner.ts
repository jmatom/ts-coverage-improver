/**
 * Domain service: detect secret-shaped strings in arbitrary text.
 *
 * Pure: no I/O, no framework. The patterns are domain knowledge — what
 * counts as "this looks like a leaked credential" is a security policy,
 * not an infrastructure detail. Lives in `domain/security/` so application
 * use cases can call it without dragging in any concrete adapter.
 *
 * Used by `RunImprovementJob` to scan the AI's logs and every written file
 * before pushing a branch — a malicious target repo may have planted
 * prompt-injection content that tricks the AI into echoing `process.env`
 * or similar.
 *
 * Patterns are intentionally conservative; we'd rather false-positive
 * (a job fails with "looks like a secret leaked") than false-negative
 * (a real `sk-ant-…` token gets committed in a generated test file).
 */
...
export class SecretScanner {
   ...
   */
  static findIn(text: string): SecretMatch | null {
