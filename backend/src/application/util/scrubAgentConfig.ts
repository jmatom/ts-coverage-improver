...
/**
 * Pre-AI hardening: delete any agent-config files an attacker-controlled
 ...
 *
 * We run this in `RunImprovementJob` after the install phase but before
...
export async function scrubAgentConfig(workdir: string): Promise<string[]> {
  ...
  await Promise.all(
    TARGETS.map(async (rel) => {
      ...
      try {
        ...
        // Truly unexpected error (permission, EBUSY) — surface it via the
        // logger but don't fail the job; agent-config scrubbing is
...
export const _AGENT_CONFIG_TARGETS = TARGETS;
