...
export class RunImprovementJob implements JobExecutor {
  ...
  private async runOneAttempt(params: {
    ...
    const targetAbs = join(params.workdir, targetTestFile);
    if (!existsSync(targetAbs)) {
      return fail(
        `Expected test file at ${targetTestFile} but it was not created. Files written: ${aiOut.writtenFiles.join(', ')}`,
        'behavioral',
      );
    }
    ...
    for (const rel of aiOut.writtenFiles) {
    ...
    const validation =
      params.mode === 'append' && params.beforeContent !== null
        ? this.deps.validator.validateAppend(targetTestFile, params.beforeContent, afterContent)
        : this.deps.validator.validateNew(targetTestFile, afterContent);
    if (!validation.ok) {
    ...
    return {
      ok: true,
      mode: params.mode,
      writtenFiles: aiOut.writtenFiles,
